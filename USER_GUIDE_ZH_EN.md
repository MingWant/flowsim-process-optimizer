...
| Queue Wait | 排队等待资源的时间 | 客户等客服接起电话的时间 |
...
| Queue Wait | 排隊等待資源的時間 | 客戶等客服接起電話的時間 |
...
| Queue Wait | Time waiting for resources | Customer waiting for an available agent |
...
**繁中**：僅顯示工作等待時間（排除非工作時間）。只計算工作時間內的佇列等待。

**EN**: Display only working wait time (excluding non-working hours). Calculate queue wait only during working hours.
...
**選項 / Options**：
- 📊 Both (Recommended) - 顯示兩個指標 / Show both metrics
- 📅 Calendar Time - 日曆時間（含週末）/ Calendar time (includes weekends)
- ⏱️ Working Time - 工作時間（純佇列效率）/ Working time (pure queue efficiency)
