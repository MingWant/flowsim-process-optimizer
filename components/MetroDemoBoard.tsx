...
import { ProcessStep, StepStats, WorkItem, DurationUnit } from '../types';
...
const formatMetricTimeInUnit = (milliseconds: number, unit: DurationUnit) => {
...
const buildFlowLayout = (flowSteps: ProcessStep[]) => {
...
const getMetroPath = (points: Array<{ x: number; y: number }>) => {
  if (points.length === 0) {
    return '';
  }
...
const getArrivalSummary = (step: ProcessStep) => {
  if (step.type !== 'start') {
    return '';
  }
...
const MetroStatPill: React.FC<{ label: string; value: string | number; tone?: string }> = ({ label, value, tone = 'text-slate-100' }) => (
...
const isBottleneckStep = (step: ProcessStep, stats?: StepStats) => {
  ...
  return (
    <div ref={boardRef} className="space-y-5 rounded-3xl bg-transparent text-slate-200 fullscreen:bg-slate-950 fullscreen:p-5 fullscreen:overflow-auto">
      ...
      {visibleFlows.map((flow) => {
        ...
        const connectionById = new Map(connectionRoutes.map((route) => [`${route.fromId}->${route.toId}`, route]));
