...
Source Rule 數值使用步驟自己的 `sourceProcessingTimeUnit`。舊配置若沒有此欄位，匯入清理會用該步驟的 Fixed processing unit 初始化，因此後續修改 Fixed unit 不會默默重新解讀既有 source rule。
...
排隊取消使用有界的 exponential hazard model。Realistic 使用 `1 - exp(-p × seconds)`；Worst-Case 保持同樣的時間一致模型，但套用壓力倍率：`1 - exp(-(2p) × seconds)`。
